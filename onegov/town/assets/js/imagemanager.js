...
(function($)
{
	RedactorPlugins.imagemanager = function()
	{
		return {
				  ...
				  success: $.proxy(function(data)
					{
						$.each(data, $.proxy(function(key, groups)
						{
							...
							$.each(groups.images, $.proxy(function(key, val)
							{
								...
								var img = $('<img class="lazyload" data-src="' + val.thumb + '" rel="' + val.image + '" title="' + thumbtitle + '" style="max-width: 100px; max-height: 75px; cursor: pointer;" />');
			...
			insert: function(e)
			{
				this.image.insert('<img class="lazyload" data-src="' + $(e.target).attr('rel') + '" alt="' + $(e.target).attr('title') + '">');
			}
		};
	};
})(jQuery);
